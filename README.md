...
このツールは、小説の原稿をePubや各種小説投稿サイト向けの原稿に変換する機能を追加します。
...
樹里「ちゃんと専用のコマンドが用意されている。作成したディレクトリに移動し、次のコマンドを入力しよう」  
...
樹里「`config`の部分は、EPUBを出力する際に必要な設定だ」  
絵子「はーい」  
