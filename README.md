...
樹里「ルートディレクトリに`.env`という設定ファイルを設置することで、細かな動作を指定することもできるぞ」  
絵子「何だか覚えることがいっぱいあるなー」  
樹里「最初のうちは特に変更する必要はないぞ。慣れてきて、細かくカスタマイズしたくなったら見直せばいい」  
絵子「うん、そうする」  
樹里「設置する際は、`.env.sample`ファイルを`.env`にリネームしよう。記述方法もそのファイルを参照してほしい」  

**EPISODES_DIR=episodes**  
原稿の保存先ディレクトリを変更することが出来ます。既定値は`episodes`です。
