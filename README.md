...
絵子「ほほう」  
樹里「ただし、他のnpmをインストールしている場合、競合する可能性もあるのでそこは注意を」  
絵子「ふむふむ」  
...
引数は、`npm run novel-build -- tcy`というように、コマンドの末尾に`--`とスペースに続けて記述します。  
...
絵子「へー」  
樹里「あとは、行間(`line-height`)、文字間(`letter-spacing`)、余白(`padding`)、背景色(`background-color`)あたりだな、調整するとしたら。もちろん、背景画像を指定したり、他にも色々調整できるぞ」  
