...
const episodes_dir = process.env.episodes_dir || 'episodes';
console.log(episodes_dir)
console.log(process.env.episodes_dir )
...
module.exports = class FileReader {
    ...
    getContent(name) {
        return new Promise( (resolve, reject) => {
            fs.readFile(path.join(episodes_dir, name), 'utf8', (err, data) => {
                if (err) reject(err);
                resolve({
                    name: name, // ファイル名
