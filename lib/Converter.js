...
module.exports = class Converter {
    ...
    static removeRuby(str) {
        return Promise.resolve(
            str.replace(patterns.ruby1, '$1').replace(patterns.ruby2, '$1')
        );
    }
    ...
    static emphasis2Bracket(str) {
        return Promise.resolve(str.replace(patterns.emphasis, '《《$2》》'));
    }
    ...
    static emphasis2HTML(str, style = 'em-sesame') {
        return Promise.resolve(
            str.replace(
                patterns.emphasis,
                (match, p1, p2, p3) => `<span class="${style}">${p2}</span>`
            )
        );
    }
    ...
    static widenAlphabet(str) {
        return Promise.resolve(
            str.replace(patterns.alphabet, (match, p1) =>
    ...
    static widenNumber(str, pattern = /(\d+)/g) {
        return Promise.resolve(
            str.replace(pattern, (match, p1) =>
                p1
                    .split('')
                    .map(n => String.fromCharCode(n.charCodeAt(0) + 65248))
                    .join('')
            )
        );
    }
    ...
    static convertSymbol(str, param = null) {
        return Promise.resolve(
            param
                ? str.replace(new RegExp(`[${param}]`, 'g'), match =>
    ...
    // markdownをHTMLに変換
    static md2HTML(str) {
        return Promise.resolve(md.render(str));
    }
