...
module.exports = class Publisher {
    ...
    constructor (contents) {
        ...
        this.params = {
            ruby: (/ruby=\w+/.test(argv) ? argv.replace(/.*ruby=(\w+).*/, '$1') : null),
            emphasis: (/emphasis=\w+/.test(argv) ? argv.replace(/.*emphasis=(\w+).*/, '$1') : null),
            alphabet: (/alphabet=\w+/.test(argv) ? argv.replace(/.*alphabet=(\w+).*/, '$1') : null),
            number: (/number=\w+/.test(argv) ? argv.replace(/.*number=(\w+).*/, '$1') : null),
            symbol: (/symbol=\W+/.test(argv) ? argv.replace(/.*symbol=(\W+).*/, '$1') : null)
        }
    }
