...
module.exports = class Epub extends Publisher {
    ...
    archive() {
        return new Promise((resolve, reject) => {
            var output = fs.createWriteStream(
                path.join(this.distDir, this.distfilename)
            );
            ...
            archive.directory(
                path.join(__dirname, templatesDir, 'META-INF'),
                'META-INF'
            );
            archive.directory(
                path.join(__dirname, templatesDir, 'item/style'),
                'item/style'
            );
            archive.directory(
                path.join(this.tempDir, 'item/xhtml'),
                'item/xhtml'
            );
            archive.file(path.join(this.tempDir, 'item/standard.opf'), {
                name: 'item/standard.opf',
            });
            archive.file(
                path.join(
                    __dirname,
                ...
                { name: 'item/navigation-documents.xhtml' }
            );
    ...
    outputContents(contents) {
        ...
        return new Promise((resolve, reject) => {
            fs.readFile(
                path.join(__dirname, templatesDir, file),
                'utf8',
                (err, data) => {
                    if (err) reject(err);
                    resolve(data);
                ...
                Promise.all(
                    contents.map(content => {
                        ...
                        fs.outputFile(
                            ...
                            console.log(`item/xhtml/p-${id}.xhtml created.`)
                        );
                    })
                );
            })
                ...
                let view = {
                    content: contents.map(content => {
                        return {
                            ...
                            title: this.convertTitle(content.title),
                        };
                    }),
    ...
    outputCover() {
        ...
        return new Promise((resolve, reject) => {
            ...
            .then(data => {
                let view = {
                    title:
                        process.env.npm_package_config_epub_title ||
            ...
            .then(([template, content]) => {
                let view = {
                    ...
                    title:
                        ...
                        process.env.npm_package_name,
                    author:
                        process.env.npm_package_config_epub_author ||
            ...
            .then(data => {
                let view = {
                    ...
                    title:
                        ...
                        process.env.npm_package_name,
                    titleFileAs:
                        process.env.npm_package_config_epub_title_file_as || '',
                    author:
                        process.env.npm_package_config_epub_author ||
                        process.env.npm_package_author,
                    authorFileAs:
                        process.env.npm_package_config_epub_author_file_as ||
                        '',
                    publisher:
                        process.env.npm_package_config_epub_publisher || '',
                    publisherFileAs:
                        process.env.npm_package_config_epub_publisher_file_as ||
                        '',
                    identifier:
